(function(){
	class Libodon {
		...
		connect(server, code){
			...
			.then(reg=>{
				registration = reg
				return get_token(server, reg, code)
			})
			.then(
			token=>{
		...
		following(id){return get_request('/api/v1/accounts/'+id+'/following')}
		relationships(...ids){
	...
	class PagedRequest {
		...
		next(){
			if(this.next_url) return get_request(this.next_url)
		}

		prev(){
			if(this.prev_url) return get_request(this.prev_url)
		}
	}
	...
	function build_paged(result){
		const link = result.headers.get('link')
		let next, prev
		if(link){
			next = (/<([^>]+)>; rel="next"/.exec(link)||{})[1]
			prev = (/<([^>]+)>; rel="prev"/.exec(link)||{})[1]
		}
		return result.json().then(body=>new PagedRequest(body,next,prev))
	}
	...
	function get_request(endpoint){
		...
		return connections[active_connection].then(conn=>{
			...
			const url = endpoint.substring(0, server.length) == server ? endpoint : server+endpoint
			...
			return fetch(url, fetchInit).then(build_paged);
		})
	}
	...
	function get_token(server, registration, code){
